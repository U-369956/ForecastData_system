...
# 批量处理目录
python /home/youqi/FWZX_forecast_DATA/EC_C1D_program/EC_v4/EC_CGYS.py \
    ...
    def _process_wind_data(self, input_files: Dict[str, str],
        ...
        try:
            ...
            for i in range(len(all_steps)):
                u_interp_func = RegularGridInterpolator(
                    (lat_src, lon_src),
                    u_cube[i],
                    bounds_error=False,
                    fill_value=np.nan
                )
                u_interp[i] = u_interp_func(points).reshape(self.n_lat_dst, self.n_lon_dst)

                v_interp_func = RegularGridInterpolator(
                    (lat_src, lon_src),
                    v_cube[i],
                    bounds_error=False,
                    fill_value=np.nan
                )
                v_interp[i] = v_interp_func(points).reshape(self.n_lat_dst, self.n_lon_dst)
            ...
            # 4. 时间插值（分段处理）- 对wspd、wdir、u、v都进行时间插值
            start_time = time.time()
            ...
            def save_netcdf_func(temp_output_path):
                with Dataset(temp_output_path, 'w') as nc:
                    ...
                    wdir_var = nc.createVariable('wdir', 'f4', ('time', 'lat', 'lon'),
                                                 zlib=True, complevel=1)
                    u_var = nc.createVariable('u', 'f4', ('time', 'lat', 'lon'),
                                              zlib=True, complevel=1)
                    v_var = nc.createVariable('v', 'f4', ('time', 'lat', 'lon'),
                                              zlib=True, complevel=1)
                    ...
                    u_var[:] = u_1h
                    u_var.units = 'm s-1'
                    u_var.long_name = '10 meter wind u component'
                    u_var.comment = 'Zonal wind component: positive eastward'

                    v_var[:] = v_1h
                    v_var.units = 'm s-1'
                    v_var.long_name = '10 meter wind v component'
                    v_var.comment = 'Meridional wind component: positive northward'

                    # 添加全局属性
                    nc.title = 'ECMWF 10m Wind Speed, Direction and UV Components (Beijing Time)'
                    nc.source = 'ECMWF Forecast'
                    nc.history = f'Created on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
                    nc.forecast_start_time_utc = base_time_utc.strftime("%Y-%m-%d %H:%M:%S UTC")
                    nc.forecast_start_time_bjt = Config.utc_to_bjt_str(base_time_utc)
            ...
            if success:
                ...
                # 返回数据
                result_data = {"wspd": wspd_1h, "wdir": wdir_1h, "u": u_1h, "v": v_1h}
                return True, result_data, times_bjt
            else:
                self.logger.error("NetCDF文件保存失败")
                return False, None, None
...
def main_cli(args=None):
    ...
    try:
        ...
        if total_stats['total_filesets'] > 0:
            ...
            success_rate = (total_stats['success_filesets'] / total_stats['total_filesets'] * 100) if total_stats[
                                                                                                          'total_filesets'] > 0 else 0
