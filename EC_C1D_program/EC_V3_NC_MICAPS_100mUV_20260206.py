...
class EC_100m_60m_Processor:
    ...
    def _process_100m_wind_data(self, input_files: Dict[str, str],
        ...
        try:
            ...
            # 3. 计算风速风向
            start_time = time.time()
            wspd_100m, wdir_100m = MeteorologicalCalculator.calculate_wind_speed_direction(u_interp, v_interp)
            ...
            if steps_3h:
                ...
                wspd_3h = wspd_100m[indices_3h]
                wdir_3h = wdir_100m[indices_3h]
            ...
            if steps_6h:
                ...
                wspd_6h = wspd_100m[indices_6h]
                wdir_6h = wdir_100m[indices_6h]
    ...
    def _process_60m_wind_data(self, input_files: Dict[str, str],
        ...
        try:
            ...
            # 计算100米风速风向
            wspd_100m, wdir_100m = MeteorologicalCalculator.calculate_wind_speed_direction(u_100m_interp, v_100m_interp)
            ...
            # 计算60米风速风向
            wspd_60m, wdir_60m = MeteorologicalCalculator.calculate_wind_speed_direction(u_60m_interp, v_60m_interp)
            ...
            if steps_3h:
                indices_3h = [i for i, step in enumerate(all_steps) if step in steps_3h]
                wspd_3h = wspd_60m[indices_3h]
                wdir_3h = wdir_60m[indices_3h]
            ...
            if steps_6h:
                indices_6h = [i for i, step in enumerate(all_steps) if step in steps_6h]
                wspd_6h = wspd_60m[indices_6h]
                wdir_6h = wdir_60m[indices_6h]
