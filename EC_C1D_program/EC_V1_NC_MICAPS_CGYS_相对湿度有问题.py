...
# 批量处理目录
python /home/youqi/FWZX_forecast_DATA/EC_C1D_program/EC_V1_NC_MICAPS_CGYS.py \
  --input-dir /home/youqi/FWZX_forecast_DATA/data_demo \
  --output-dir /home/youqi/FWZX_forecast_DATA/output \
  --elements 'TEM,WIND,DPT,PRS,RH,GUST' \
  --save-micaps4 \
...
class MeteorologicalCalculator:
    ...
    @staticmethod
    def calculate_relative_humidity(temp_k: np.ndarray, prs_pa: np.ndarray, dpt_k: np.ndarray) -> np.ndarray:
        """
        计算相对湿度
        ...
        """
        # 转换为摄氏度
        temp_c = MeteorologicalCalculator.kelvin_to_celsius(temp_k)
        dpt_c = MeteorologicalCalculator.kelvin_to_celsius(dpt_k)
        prs_hpa = prs_pa / 100.0  # Pa转换为hPa
        
        # 计算饱和水汽压 (hPa)
        e_s = 6.112 * np.exp(17.62 * temp_c / (temp_c + 243.12))
        
        # 计算实际水汽压 (hPa)
        e = 6.112 * np.exp(17.62 * dpt_c / (dpt_c + 243.12))
        ...
        # 处理NaN值
        rh = np.where(np.isnan(rh), np.nan, rh)
...
class ECProcessor:
    ...
    def process_element(self, element: str, input_files: Dict[str, str], 
        ...
        try:
            ...
            if element == "RH":
                # 相对湿度需要特殊处理
                success, result_data, times_bjt = self._process_relative_humidity(
    ...
    def _process_relative_humidity(self, input_files: Dict[str, str],
                                    base_time_utc: datetime, output_path: str) -> Tuple[bool, Dict[str, np.ndarray], List[datetime]]:
            """处理相对湿度数据（需要温度、气压、露点温度）"""
            try:
                self.logger.info("处理相对湿度数据（需要温度、气压、露点温度）")
                
                # 检查输入文件
                temp_file = input_files.get("temp")
                prs_file = input_files.get("prs")
                dpt_file = input_files.get("dpt")
                
                if not temp_file or not prs_file or not dpt_file:
                    self.logger.error("相对湿度处理需要温度、气压和露点温度文件")
                    return False, None, None
                
                # 分别处理温度、气压、露点温度（温度已转换为摄氏度）
                self.logger.info("1. 处理温度数据...")
                temp_success, temp_data, times_bjt = self._process_scalar_data("TEM", {"value": temp_file}, base_time_utc, None)
                
                self.logger.info("2. 处理气压数据...")
                prs_success, prs_data, _ = self._process_scalar_data("PRS", {"value": prs_file}, base_time_utc, None)
                
                self.logger.info("3. 处理露点温度数据...")
                dpt_success, dpt_data, _ = self._process_scalar_data("DPT", {"value": dpt_file}, base_time_utc, None)
                
                if not (temp_success and prs_success and dpt_success):
                    self.logger.error("基础要素处理失败，无法计算相对湿度")
                    return False, None, None
                
                # 检查数据一致性
                if (temp_data["temp"].shape != prs_data["prs"].shape or 
                    temp_data["temp"].shape != dpt_data["dpt"].shape):
                    self.logger.error("基础要素数据形状不一致")
                    return False, None, None
                
                # 注意：相对湿度计算需要开尔文温度
                # 将摄氏度转换回开尔文用于计算
                self.logger.info("4. 将温度转换回开尔文用于相对湿度计算...")
                temp_k = temp_data["temp"] + 273.15  # 摄氏度转开尔文
                dpt_k = dpt_data["dpt"] + 273.15    # 摄氏度转开尔文
                
                # 计算相对湿度
                start_time = time.time()
                self.logger.info("5. 计算相对湿度...")
                
                rh_data = MeteorologicalCalculator.calculate_relative_humidity(
                    temp_k, prs_data["prs"], dpt_k
                )
                ...
                with Dataset(output_path, 'w') as nc:
                    ...
                    rh_var.formula = 'e_s = 6.112 × exp[17.62 × T / (T + 243.12)]; e = 6.112 × exp[17.62 × Td / (Td + 243.12)]; RH = (e / e_s) × 100%'
                    rh_var.temperature_units = 'Temperature input in Celsius for RH calculation'
                    ...
                    nc.latitude_order = 'south to north (ascending)'
                    nc.time_interpolation = '分段线性插值: 0-72小时(3h→1h), 73-240小时(6h→1h)'
                    nc.temperature_units_note = 'Temperature and dew point converted to Celsius for output, converted back to Kelvin for RH calculation'
...
def create_file_sets_from_directory(input_dir: str, element: str) -> List[Dict[str, str]]:
    ...
    if element == "WIND":
        ...
        # 相对湿度需要温度、气压、露点温度文件
        print(f"搜索RH相关文件: 温度(TEM)、气压(PRS)、露点(DPT)")
        ...
        # 查找气压文件 (地面气压)
        prs_files = []
        for pattern in ["*SP*.grib*", "*PRS*.grib*", "*sp*.grib*", "*prs*.grib*", "*msl*.grib*"]:
            prs_files.extend(glob(os.path.join(input_dir, pattern)))
        prs_files = sorted(set(prs_files))
        
        # 查找露点温度文件 (2米露点温度)
        dpt_files = []
        for pattern in ["*2D*.grib*", "*DPT*.grib*", "*2d*.grib*", "*dpt*.grib*", "*DEW*.grib*"]:
            dpt_files.extend(glob(os.path.join(input_dir, pattern)))
        dpt_files = sorted(set(dpt_files))
        
        print(f"找到温度文件: {len(temp_files)} 个")
        print(f"找到气压文件: {len(prs_files)} 个")
        print(f"找到露点文件: {len(dpt_files)} 个")
        ...
        for temp_file in temp_files:
            ...
            if time_match:
                ...
                if time_str not in file_groups:
                    file_groups[time_str] = {'temp': None, 'prs': None, 'dpt': None}
                file_groups[time_str]['temp'] = temp_file
        
        # 匹配气压文件
        for prs_file in prs_files:
            base_name = os.path.basename(prs_file)
            time_match = re.search(r'\d{10}', base_name)
            if time_match:
                time_str = time_match.group(0)
                if time_str in file_groups:
                    file_groups[time_str]['prs'] = prs_file
        
        # 匹配露点文件
        for dpt_file in dpt_files:
            base_name = os.path.basename(dpt_file)
            time_match = re.search(r'\d{10}', base_name)
            if time_match:
                time_str = time_match.group(0)
                if time_str in file_groups:
                    file_groups[time_str]['dpt'] = dpt_file
        
        # 创建完整的文件集
        for time_str, files in file_groups.items():
            if files['temp'] and files['prs'] and files['dpt']:
                file_sets.append({
                    "temp": files['temp'],
                    "prs": files['prs'],
                    "dpt": files['dpt']
                })
                print(f"找到RH文件集 {time_str}: TEM={os.path.basename(files['temp'])}, "
                      f"PRS={os.path.basename(files['prs'])}, DPT={os.path.basename(files['dpt'])}")
            else:
                missing = []
                if not files['temp']:
                    missing.append('TEM')
                ...
                if not files['dpt']:
                    missing.append('DPT')
                print(f"警告: 时间 {time_str} 缺少文件: {', '.join(missing)}")
            ...
            try:
                ...
                if args.input_dir is None and len(elements) == 1:
                    ...
                    if element == "WIND":
                        ...
                        # 相对湿度需要从命令行获取多个文件
                        logger.error("相对湿度要素需要多个输入文件，请使用--input-dir参数")
                        element_success = False
                        total_stats['failed_elements'] += 1
                        continue
                    else:
