...
class MICAPS4Writer:
    ...
    def create_micaps4_filename(base_time: datetime, forecast_hour: int,
        ...
        return f"{local_time.strftime('%Y%m%d%H')}.10WIND.{forecast_hour:03d}"
    ...
    def _process_core_with_micaps4(self, u_file: str, v_file: str, 
        ...
        try:
            ...
            with pygrib.open(u_file) as grbs_u, pygrib.open(v_file) as grbs_v:
                ...
                self.logger.info(f"预报时效范围: 0-{max_step}小时")
                ...
                # 读取3小时间隔数据 (0-72小时)
                if steps_3h:
                    self.logger.info(f"读取0-72小时数据: {len(steps_3h)}个时次 (3小时间隔)")
                    u_3h = []
                    v_3h = []
                    
                    for step in steps_3h:
                        # 读取U分量
                ...
                # 读取6小时间隔数据 (72-240小时)
                if steps_6h:
                    self.logger.info(f"读取72-240小时数据: {len(steps_6h)}个时次 (6小时间隔)")
                    u_6h = []
                    v_6h = []
            ...
            self.logger.info(f"时效分布: 0-72小时: {len(steps_3h)}个时次, 72-240小时: {len(steps_6h)}个时次")
            ...
            # 确定最大预报时效
            max_forecast_hour = max(all_steps)
            ...
            # 第一段: 0-72小时 (3小时间隔)
            if steps_3h:
                self.logger.info(f"处理0-72小时时间插值 (3小时间隔 → 1小时间隔)")
                ...
                # 创建插值器
                hours_3h = steps_3h  # 预报时效（小时）
                hours_new_3h = np.arange(0, 72 + 1, 1.0)  # 0-72小时，1小时间隔
            ...
            # 第二段: 72-240小时 (6小时间隔)
            if steps_6h:
                self.logger.info(f"处理72-240小时时间插值 (6小时间隔 → 1小时间隔)")
                ...
                # 创建插值器
                hours_6h = steps_6h  # 预报时效（小时）
                
                # 确定插值范围: 从72+1=73小时开始，到最大预报时效
                start_hour_6h = 73  # 从73小时开始，避免与72小时重复
            ...
            if wspd_1h_segments:
                ...
                self.logger.info(f"预报时效范围: {hours_new[0]}-{hours_new[-1]}小时")
            else:
