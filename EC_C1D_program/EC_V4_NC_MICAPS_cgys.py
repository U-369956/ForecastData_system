...
# 批量处理目录
python /home/youqi/FWZX_forecast_DATA/EC_C1D_program/EC_V4_NC_MICAPS_cgys.py \
...
class MICAPS4Writer:
    ...
    # 注：write_micaps4_vector_file 方法中的create_micaps4_filename调用会自动使用新的命名规则
...
class ECProcessor:
        ...
        try:
            ...
            with pygrib.open(data_file) as grbs:
                ...
                for step in steps:
                    ...
                    if (lat_src[-1] > lat_src[0] and lats[-1, 0] < lats[0, 0]) or \
                       (lat_src[-1] < lat_src[0] and lats[-1, 0] > lats[0, 0]):
                        data = data[::-1, :]
            ...
            data_interp = self._spatial_interpolation(data_cube, lat_src, lon_src)
            ...
            if data_1h_segments:
                ...
                times_bjt = self._create_time_series(base_time_utc, hours_new)
            ...
            def save_netcdf_func(temp_output_path):
                with Dataset(temp_output_path, 'w') as nc:
                    self._setup_netcdf_dimensions(nc, len(times_bjt))
                    self._setup_netcdf_coordinates(nc, times_bjt)
                    ...
                    self._setup_netcdf_global_attrs(
                        nc, base_time_utc, 
                        f'ECMWF {element_config["description"]} (Beijing Time)'
                    )
                return True
    ...
    def _process_wind_data(self, input_files: Dict[str, str],
        ...
        try:
            ...
            u_interp = self._spatial_interpolation(u_cube, lat_src, lon_src)
            v_interp = self._spatial_interpolation(v_cube, lat_src, lon_src)
            ...
            def save_netcdf_func(temp_output_path):
                with Dataset(temp_output_path, 'w') as nc:
                    ...
                    self._setup_netcdf_global_attrs(
                        nc, base_time_utc, 
                        'ECMWF 10m Wind Speed and Direction (Beijing Time)'
                    )
                return True
    ...
    def _process_relative_humidity_from_temperature(self, temp_data: np.ndarray,
        ...
        try:
            ...
            hours_array = np.arange(n_times, dtype=float)
            times_bjt = self._create_time_series(base_time_utc, hours_array)
            
            start_time = time.time()
            
            def save_netcdf_func(temp_output_path):
                with Dataset(temp_output_path, 'w') as nc:
                    self._setup_netcdf_dimensions(nc, n_times)
                    self._setup_netcdf_coordinates(nc, times_bjt)
                    ...
                    self._setup_netcdf_global_attrs(
                        nc, base_time_utc, 
                        'ECMWF 2m Relative Humidity (Beijing Time)',
                        'Relative humidity calculated from temperature data using simplified algorithm'
                    )
                return True
    ...
    def _create_time_series(self, base_time_utc: datetime, hours: np.ndarray) -> List[datetime]:
        """创建时间序列"""
        return [base_time_utc + timedelta(hours=float(h)) + Config.TIMEZONE_SHIFT for h in hours]
    
    def _spatial_interpolation(self, data_cube: np.ndarray, lat_src: np.ndarray, lon_src: np.ndarray) -> np.ndarray:
        """空间插值到目标网格"""
    ...
    def _setup_netcdf_dimensions(self, nc: Dataset, n_times: int):
        """设置NetCDF维度"""
        nc.createDimension('time', n_times)
        nc.createDimension('lat', self.n_lat_dst)
        nc.createDimension('lon', self.n_lon_dst)
    
    def _setup_netcdf_coordinates(self, nc: Dataset, times_bjt: List[datetime]):
        """设置NetCDF坐标变量"""
    ...
    def _setup_netcdf_global_attrs(self, nc: Dataset, base_time_utc: datetime, title: str, 
        ...
        if calculation_method:
            nc.calculation_method = calculation_method
    ...
    def _save_micaps4_files(self, element: str, result_data: Dict[str, np.ndarray],
            ...
            if wspd_data is not None and wdir_data is not None:
                for i in range(wspd_data.shape[0]):
                    ...
                    filename = MICAPS4Writer.create_micaps4_filename(
                        ...
                        element=element,  # 虽然文件名中不包含，但传入用于内部处理
                        model_name="ECMWF"
                    )
        ...
        self.logger.info(f"输出目录: {element_output_dir}")
